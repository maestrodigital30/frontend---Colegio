...
export default function RankingDocentePage() {
          ...
          {ranking.length === 0 ? (
            ...
            <div className="divide-y divide-slate-200">
              {ranking.map((r, idx) => (
                ...
                >
                  <span className={`text-2xl font-bold w-10 text-center ${idx === 0 ? 'text-primary-600' : idx === 1 ? 'text-slate-500' : idx === 2 ? 'text-primary-700' : 'text-slate-500'}`}>
                    {idx < 3 ? ['\u{1F947}', '\u{1F948}', '\u{1F949}'][idx] : `#${idx + 1}`}
                  </span>
                  <AvatarAlumno identidad={identidadDeRanking(r)} size="sm" />
                  <div className="flex-1">
                    <p className="text-sm font-medium text-slate-700">{r.apellidos}, {r.nombres}</p>
                  </div>
