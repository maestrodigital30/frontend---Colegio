...
export default function HistorialTriviaDocentePage() {
                  ...
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 flex-wrap">
                        <span className={`px-2 py-0.5 rounded-lg text-xs font-medium ${estadoColor[p.estado_partida] || estadoColor[p.estado] || 'bg-slate-50 text-slate-500'}`}>
                      ...
                      <div className="space-y-2">
                        {participantesOrdenados(p.tbl_trivia_participantes).map((part, idx) => (
                          <button
                            ...
                            className={`w-full flex items-center gap-3 p-2.5 rounded-lg transition-colors cursor-pointer text-left ${part.es_ganador ? 'bg-sky-100 border-2 border-sky-300 hover:bg-sky-200' : 'bg-white border border-slate-200 hover:bg-slate-50'}`}
                          >
                            <span className="text-sm font-bold w-7 text-center text-black">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {part.numero_equipo && (
                                <span className="text-xs text-slate-500">{p.modalidad === MODALIDADES_TRIVIA.PAREJAS ? 'Pareja' : 'Grupo'} {part.numero_equipo}</span>
                              )}
                            </div>
                      ...
                      {opciones.map(op => {
                        ...
                        return (
                          <div key={op.id} className={`flex items-center gap-1.5 px-2 py-1 rounded border text-xs ${estiloOpcion}`}>
                            {esCorrecta && <HiCheckCircle className="w-3.5 h-3.5 text-emerald-600 flex-shrink-0" />}
                            {fueSeleccionada && !esCorrecta && <HiXCircle className="w-3.5 h-3.5 text-rose-600 flex-shrink-0" />}
