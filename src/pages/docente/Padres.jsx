...
export default function PadresDocentePage() {
  ...
  const columnas = [
    ...
    { key: 'hijos', label: 'Alumnos', headerClassName: 'bg-primary-200 text-primary-800', cellClassName: 'bg-primary-100 text-primary-800 font-medium', render: (f) => f.tbl_padres_alumnos?.filter(pa => pa.estado === 1).map(pa => `${pa.tbl_alumnos?.nombres} ${pa.tbl_alumnos?.apellidos}`).join(', ') || '-' },
  ...
  const abrirCrear = () => { setEditando(null); setForm({ nombres: '', apellidos: '', telefono: '', correo: '', es_principal: false }); setErrores({}); setErrorGeneral(''); setModal(true); };
  const abrirEditar = (p) => { setEditando(p); setForm({ nombres: p.nombres, apellidos: p.apellidos, telefono: p.telefono || '', correo: p.correo || '', es_principal: p.es_contacto_principal || false }); setErrores({}); setErrorGeneral(''); setModal(true); };
  const abrirVincular = (p) => { setPadreSel(p); setAlumnoVinc(''); setParentesco('padre'); setModalVinc(true); };
  ...
  const handleVincular = async () => {
    ...
    try {
      await apiClient.post(`/padres/${padreSel.id}/vincular-alumno`, { id_alumno: parseInt(alumnoVinc), parentesco });
  ...
  const handleToggleEstado = async (p) => {
    if (p.estado === 1) {
      ...
      try { await inactivar(p.id); } catch { toast.error('Error'); }
    } else {
      if (!confirm('¿Activar?')) return;
      try { await activar(p.id); } catch { toast.error('Error'); }
    }
  };
  ...
  return (
    <div className="animate-fade-up" style={{ fontSize: '115%' }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <p className="text-xs font-display font-medium text-secondary-600 uppercase tracking-widest mb-1">Comunidad</p>
      ...
      <Tabla columnas={columnas} datos={datos} filtros={filtros} accionesClassName="bg-primary-200 text-primary-800" acciones={(fila) => (
        <>
          <button onClick={() => abrirEditar(fila)} className="p-1 text-primary-400 hover:bg-slate-50 rounded"><HiPencil className="w-4 h-4" /></button>
          <button onClick={() => abrirVincular(fila)} className="p-1 text-secondary-600 hover:bg-slate-50 rounded" title="Vincular"><HiLink className="w-4 h-4" /></button>
          {fila.estado === 1 ? (
      ...
      <Modal abierto={modal} cerrar={() => setModal(false)} titulo={editando ? 'Editar Padre' : 'Nuevo Padre'}>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <InputCampo label="Teléfono" name="telefono" value={form.telefono} onChange={handleChange} placeholder="51999999999" required error={errores.telefono} />
      ...
      <Modal abierto={modalVinc} cerrar={() => setModalVinc(false)} titulo="Vincular Alumno">
        <div className="space-y-3">
          <InputCampo label="Alumno" name="alumno" type="select" value={alumnoVinc} onChange={(e) => setAlumnoVinc(e.target.value)} options={alumnos.filter(a => a.estado === 1).map(a => ({ value: a.id.toString(), label: `${a.apellidos}, ${a.nombres}` }))} />
          <InputCampo label="Parentesco" name="parentesco" type="select" value={parentesco} onChange={(e) => setParentesco(e.target.value)} options={[
            { value: 'padre', label: 'Padre' }, { value: 'madre', label: 'Madre' }, { value: 'apoderado', label: 'Apoderado' }, { value: 'tutor', label: 'Tutor' },
          ]} />
          <div className="flex justify-end gap-2 pt-3">
