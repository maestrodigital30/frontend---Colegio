...
import { HiPencil, HiBan, HiCheckCircle, HiRefresh, HiUser, HiClipboardList } from 'react-icons/hi';
...
export default function AlumnosDocentePage() {
  ...
  const [form, setForm] = useState({ nombres: '', apellidos: '', dni: '', fecha_nacimiento: '', genero: '', direccion: '', correo_acceso: '', contrasena_acceso: '' });
  ...
  const columnas = [
    ...
    { key: 'acceso', label: 'Acceso', headerClassName: 'bg-primary-200 text-primary-800', cellClassName: 'bg-primary-100', render: (f) => f.tbl_usuarios?.correo
        ? <span className="text-xs text-secondary-600 font-mono">{f.tbl_usuarios.correo}</span>
  ...
  const abrirCrear = () => {
    setEditando(null);
    setForm({ nombres: '', apellidos: '', dni: '', fecha_nacimiento: '', genero: '', direccion: '', correo_acceso: '', contrasena_acceso: '' });
    setFotoPreview(null);
    setArchivoFoto(null);
    setErrores({});
    setModal(true);
  ...
  const abrirEditar = (a) => {
    setEditando(a);
    setForm({
      nombres: a.nombres || '', apellidos: a.apellidos || '', dni: a.dni || '',
      fecha_nacimiento: a.fecha_nacimiento?.split('T')[0] || '', genero: a.genero || '', direccion: a.direccion || '',
      correo_acceso: a.tbl_usuarios?.correo || '', contrasena_acceso: '',
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editando) {
        ...
        const nuevoAlumno = await crear(form);
        id = nuevoAlumno.id;
  ...
  return (
    <div className="animate-fade-up" style={{ fontSize: '115%' }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <p className="text-xs font-display font-medium text-secondary-600 uppercase tracking-widest mb-1">Gestión</p>
          <h1 className="text-3xl font-display font-bold text-slate-800">Mis Alumnos</h1>
        </div>
      ...
      <Tabla columnas={columnas} datos={datos} filtros={filtros} accionesClassName="bg-primary-200 text-primary-800" acciones={(fila) => (
        <>
          ...
          <button onClick={() => abrirAsignarCursos(fila)} className="p-1 text-secondary-600 hover:bg-slate-50 rounded" title="Asignar Cursos">
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto">
          {cursos.filter(c => c.estado === 1).map(c => (
            <label key={c.id} className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-lg cursor-pointer">
              <input type="checkbox" checked={cursosSeleccionados.includes(c.id.toString())} onChange={() => toggleCurso(c.id)} className="w-4 h-4 text-primary rounded" />
