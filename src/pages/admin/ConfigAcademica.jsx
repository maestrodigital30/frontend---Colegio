...
export default function ConfigAcademicaPage() {
  ...
  const [formEsquema, setFormEsquema] = useState({
    tipo_calificacion: TIPOS_CALIFICACION.NUMERICO,
    formula: 'promedio',
    cantidad_periodos: 4,
    periodos: [],
    componentes: [],
  ...
  const agregarComponente = () => {
    setFormEsquema(prev => ({
      ...prev,
      componentes: [...prev.componentes, { nombre_componente: '', peso_porcentaje: 0 }],
    }));
  };
  ...
  const crearEsquema = async () => {
    ...
    if (formEsquema.periodos.length === 0) return toast.error('Genere los periodos primero');
    ...
    try {
      ...
      toast.error(err.response?.data?.error || 'Error al crear esquema');
    }
  };
  ...
  const actualizarFormula = async () => {
    ...
    try {
      ...
      toast.success('Fórmula actualizada y notas recalculadas');
    } catch (err) {
  ...
  const grados = useMemo(() => {
    const unicos = [...new Set(cursos.map(d => d.grado).filter(Boolean))];
            ...
            {esquema && !modoEdicion ? (
              <>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div><span className="text-sm text-slate-400">Tipo:</span> <span className="font-medium text-slate-600">{esquema.tipo_calificacion === 'letras' ? 'Letras' : 'Numérico'}</span></div>
                  {esquema.tipo_calificacion === 'letras' ? (
                    <div><span className="text-sm text-slate-400">Escala:</span> <span className="font-medium text-slate-600">{esquema.escala_letras || '-'}</span></div>
                  ) : (
                    <div><span className="text-sm text-slate-400">Fórmula:</span> <span className="font-medium text-slate-600">{esquema.formula}</span></div>
                  )}
                </div>

                <h3 className="font-medium text-slate-600 mb-2">Periodos</h3>
                ...
                <h3 className="font-medium text-slate-600 mb-2">Componentes de Nota</h3>
                <div className="space-y-2">
                  {esquema.tbl_componentes_nota?.map(c => (
                    <div key={c.id} className="flex items-center gap-4 p-2 bg-slate-50 rounded-lg">
                      ...
                      {esquema.tipo_calificacion !== 'letras' && (
                        <span className="text-sm font-medium text-slate-800">{c.peso_porcentaje}%</span>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <>
                <h2 className="text-lg font-display font-semibold text-slate-800 mb-4">{modoEdicion ? 'Editar Esquema de Calificación' : 'Crear Esquema de Calificación'}</h2>
                <div className="space-y-4">
                  <InputCampo label="Tipo de Calificación" name="tipo" type="select" value={formEsquema.tipo_calificacion} onChange={(e) => setFormEsquema(prev => ({ ...prev, tipo_calificacion: e.target.value }))} options={[
                    { value: TIPOS_CALIFICACION.NUMERICO, label: 'Numérico' },
                    { value: TIPOS_CALIFICACION.LETRAS, label: 'Letras' },
                  ]} />
                  {esLetras ? (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-2">
                        {ESCALAS_PRESET.map(p => (
                          <button key={p.value} type="button" onClick={() => setFormEsquema(prev => ({ ...prev, escala_letras: p.value }))}
                            className={`px-3 py-1 rounded-lg text-xs border transition-colors ${formEsquema.escala_letras === p.value ? 'bg-primary-50 border-primary-300 text-primary-700' : 'bg-slate-50 border-slate-200 text-slate-600 hover:border-slate-300'}`}>
                            {p.label}
                          </button>
                        ))}
                      </div>
                      <input type="text" value={formEsquema.escala_letras} onChange={(e) => setFormEsquema(prev => ({ ...prev, escala_letras: e.target.value }))} placeholder="Ej: AD,A,B,C o A+,A,A-,B+,B" className="w-full px-3 py-2 border border-slate-200 bg-slate-50 text-slate-800 rounded-lg text-sm" />
                      <p className="text-xs text-slate-400 mt-1">Separe las letras con comas. La nota final se ingresa manualmente.</p>
                    </div>
                  ) : (
                    <InputCampo label="Fórmula" name="formula" value={formEsquema.formula} onChange={(e) => setFormEsquema(prev => ({ ...prev, formula: e.target.value }))} placeholder="promedio" />
                  )}

                  <div className="flex items-end gap-3">
                    <InputCampo label="Cantidad de Periodos" name="cant" type="number" value={formEsquema.cantidad_periodos} onChange={(e) => setFormEsquema(prev => ({ ...prev, cantidad_periodos: parseInt(e.target.value) || 4 }))} />
                    <Boton tipo="outline" onClick={generarPeriodos}>Generar Periodos</Boton>
                  </div>

                  {formEsquema.periodos.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {formEsquema.periodos.map((p, i) => (
                        <span key={i} className="px-3 py-1 bg-secondary-50 text-secondary-600 border border-secondary-200 rounded-lg text-sm">{p.nombre}</span>
                      ))}
                    </div>
                  )}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="font-medium text-slate-600">Componentes de Nota</h3>
                      <Boton tipo="outline" onClick={agregarComponente}>+ Componente</Boton>
                    </div>
                    {formEsquema.componentes.map((c, i) => (
                      <div key={i} className="flex items-center gap-2 mb-2">
                        <input type="text" value={c.nombre_componente} onChange={(e) => actualizarComponente(i, 'nombre_componente', e.target.value)} placeholder="Ej: Examen" className="flex-1 px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50 text-slate-800 placeholder-slate-500" />
                        {!esLetras && (
                          <>
                            <input type="number" value={c.peso_porcentaje} onChange={(e) => actualizarComponente(i, 'peso_porcentaje', e.target.value)} placeholder="%" className="w-20 px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50 text-slate-800" />
                            <span className="text-sm text-slate-400">%</span>
                          </>
                        )}
                        <button onClick={() => eliminarComponente(i)} className="p-1 text-rose-400 hover:bg-slate-50 rounded">X</button>
                      </div>
                    ))}
                  </div>
