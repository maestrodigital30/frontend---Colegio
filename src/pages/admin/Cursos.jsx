...
export default function CursosPage() {
  ...
  const [form, setForm] = useState({ nombre: '', descripcion: '', id_docente: '', id_periodo_escolar: '', grado: '', seccion: '' });
  ...
  const columnas = [
    ...
    { key: 'docente', label: 'Docente', headerClassName: 'bg-violet-200 text-violet-800', cellClassName: 'bg-violet-100 text-violet-800 font-medium', render: (f) => f.tbl_perfiles_docente?.tbl_usuarios ? `${f.tbl_perfiles_docente.tbl_usuarios.nombres} ${f.tbl_perfiles_docente.tbl_usuarios.apellidos}` : '-' },
    { key: 'periodo', label: 'Periodo', headerClassName: 'bg-teal-200 text-teal-800', cellClassName: 'bg-teal-100 text-teal-800 font-medium', render: (f) => f.tbl_periodos_escolares?.nombre || '-' },
  ...
  const periodosOpciones = useMemo(() => {
    const unicos = [...new Map(datos.map(d => [d.tbl_periodos_escolares?.nombre, d.tbl_periodos_escolares]).filter(([k]) => k)).values()];
  ...
  const abrirCrear = () => {
    setEditando(null);
    setForm({ nombre: '', descripcion: '', id_docente: '', id_periodo_escolar: '', grado: '', seccion: '' });
    setModal(true);
  };
  ...
  const abrirEditar = (c) => {
    ...
    setForm({
      ...
      id_docente: c.id_docente?.toString() || '',
    ...
    <div className="animate-fade-up" style={{ fontSize: '115%' }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-3xl font-display font-bold text-slate-800">Cursos</h1>
        </div>
      ...
      <Modal abierto={modal} cerrar={() => setModal(false)} titulo={editando ? 'Editar Curso' : 'Nuevo Curso'}>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <InputCampo label="Docente" name="id_docente" type="select" value={form.id_docente} onChange={handleChange} required options={docentes.filter(d => d.estado === 1).map(d => ({
            value: d.id.toString(),
            label: `${d.tbl_usuarios?.nombres || ''} ${d.tbl_usuarios?.apellidos || ''}`.trim(),
          }))} />
